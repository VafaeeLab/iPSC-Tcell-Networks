...
#change dir to ishi2d processing dir
cat("Changing dir to ishi2d processing dir...\n")
setwd("iPSC_dataset_processing/ishi_2d")

### now that we have our gene list, we can perform network analysis with hdWGCNA
seurat_obj <- SetupForWGCNA(
  seurat_obj,
  wgcna_name = "ishi2d_optimisation",
...
for (i in 1:nrow(param_grid)) {
  ...
  new_row <- tryCatch({
    ...
    soft_power_val = seurat_obj_loop@misc$ishi2d_optimisation$wgcna_params$power
    
    exp <- GetDatExpr(seurat_obj_loop, wgcna_name = "ishi2d_optimisation")
...
write.csv(results_df, file = "ishi2d_opt_results.csv")

results_df <- read.csv(file = "ishi2d_opt_results.csv")
