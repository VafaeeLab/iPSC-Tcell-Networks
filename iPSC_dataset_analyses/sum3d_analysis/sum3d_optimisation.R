...
seurat_obj <- subset(seurat_obj, subset = orig.ident == "ATO_T6")
...
#change dir to g2g_ato processing dir
cat("Changing dir to g2g_ato processing dir...\n")
setwd("iPSC_dataset_processing/g2g_ATO")

### now that we have our gene list, we can perform network analysis with hdWGCNA
seurat_obj <- SetupForWGCNA(
  seurat_obj,
  wgcna_name = "g2g_ATO_optimisation",
...
phenotypes <- unique(seurat_obj$maturity)
...
for (i in 1:nrow(param_grid)) {
  ...
  new_row <- tryCatch({
    ...
    seurat_obj_loop <- SetDatExpr(
      seurat_obj_loop,
      group_name = phenotypes, # using all cell types
    ...
    soft_power_val = seurat_obj_loop@misc$g2g_ATO_optimisation$wgcna_params$power
    
    exp <- GetDatExpr(seurat_obj_loop, wgcna_name = "g2g_ATO_optimisation")
    ...
    seurat_obj_loop <- ModuleEigengenes(seurat_obj_loop, group.by.vars = covariates) #performing integration against covariates
...
write.csv(results_df, file = "g2g_ATO_opt_results.csv")

results_df <- read.csv("g2g_ATO_opt_results.csv")
