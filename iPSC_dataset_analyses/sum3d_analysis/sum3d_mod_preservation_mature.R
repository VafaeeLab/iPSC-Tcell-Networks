#in this script we are loading up our processed reference and g2g_ATO rds files, 
...
setwd("~/ipsc_project/R_scripts/iPSC_analysis/iPSC_dataset_processing/g2g_ATO")
...
#our iPSC query data
seurat_query <- readRDS(file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/g2g_ATO/ATO_integrated.rds")

#subset to non-tnf cells
seurat_query <- subset(seurat_query, subset = orig.ident == "ATO_T6")
...
#setting the covariates to integrate against
covariates <- c("cell_line")
#phenotypes <- unique(seurat_query$maturity)
...
seurat_query <- MetacellsByGroups(
  ...
  group.by = c("maturity", covariates), 
  #maturity = mature and immature T, covariates = cell_line (omitting orig.ident as tnf population is too small)
  k = k_val,
  max_shared = max_shared_val,
  ident.group = 'maturity',
...
seurat_query <- SetDatExpr(
  seurat_query,
  group_name = "mature_T", # only using mature T cells
  group.by='maturity', # the metadata column containing the cell type info. This same column should have also been used in MetacellsByGroups
...
seurat_query <- ModulePreservation(
  seurat_query,
  seurat_ref = seurat_ref,
  name="conv_T_preservation_mature",
  verbose=3,
  seed = 12345,
  parallel = FALSE,
  n_permutations=250
...
saveRDS(seurat_query, file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/g2g_ATO/ATO_integrated_NO_TNF.rds")
seurat_query <- readRDS(file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/g2g_ATO/ATO_integrated_NO_TNF.rds")
