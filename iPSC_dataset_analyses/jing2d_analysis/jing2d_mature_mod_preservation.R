#in this script we are loading up our processed reference and ez_T rds files, 
...
#but only focusing on metacells derived from a particular subpopulation

library(Seurat)
library(hdWGCNA)

setwd("~/ipsc_project/R_scripts/iPSC_analysis/iPSC_dataset_processing/EZ_T")
...
#our iPSC query data
seurat_query <- readRDS(file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/EZ_T/processed/EZ_T.rds")

#DimPlot(seurat_query, group.by = "phenotype", reduction = "harmony.umap")
...
phenotype_list <- c("mature_T", "mature_activated_T")

## Firstly set up for WGCNA
seurat_query <- SetupForWGCNA(
  seurat_query,
  wgcna_name = "conv_T_mod_projection_mature",
...
seurat_query <- MetacellsByGroups(
  ...
  group.by = c("orig.ident", "phenotype"), #orig.ident = batch id
  k = k_val,
  max_shared = max_shared_val,
  ident.group = 'phenotype',
...
seurat_query <- SetDatExpr(
  seurat_query,
  group_name = phenotype_list, # in this run we are selecting all mature T cells
  group.by='phenotype', # the metadata column containing the cell type info. This same column should have also been used in MetacellsByGroups
...
saveRDS(seurat_query, file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/EZ_T/processed/EZ_T.rds")
seurat_query <- readRDS(file = "/g/data/yr31/hb1618/iPSC_project/datasets/iPSC/EZ_T/processed/EZ_T.rds")
