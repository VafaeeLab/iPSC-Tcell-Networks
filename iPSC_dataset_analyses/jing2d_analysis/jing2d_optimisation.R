...
#change dir to EZ_T processing dir
cat("Changing dir to EZ_T processing dir...\n")
setwd("iPSC_dataset_processing/EZ_T")

### now that we have our gene list, we can perform network analysis with hdWGCNA
seurat_obj <- SetupForWGCNA(
  seurat_obj,
  wgcna_name = "EZ_T_optimisation",
...
phenotype_list <- unique(seurat_obj$phenotype)
...
for (i in 1:nrow(param_grid)) {
  ...
  new_row <- tryCatch({
    ...
    seurat_obj_loop <- SetDatExpr(
      seurat_obj_loop,
      group_name = phenotype_list, # the name of the groups of interest in the group.by column
    ...
    soft_power_val = seurat_obj_loop@misc$EZ_T_optimisation$wgcna_params$power
    
    exp <- GetDatExpr(seurat_obj_loop, wgcna_name = "EZ_T_optimisation")
    ...
    seurat_obj_loop <- ModuleEigengenes(seurat_obj_loop, group.by.vars = "orig.ident") #integrates based on orig.ident
    seurat_obj_loop <- ModuleConnectivity(seurat_obj_loop,
                                          group.by = "phenotype", 
                                          group_name = phenotype_list,
                                          harmonized = TRUE)
...
write.csv(results_df, file = "EZ_T_opt_results.csv")

results_df <- read.csv("EZ_T_opt_results.csv")
